...
export class ImageService extends BaseService {
    ...
    public async GetImageSlider(): Promise<ImageModel[]> {
        try {
            ...
            const imagesSilder: Image[] = await repositoryImage.createQueryBuilder('image')
            .where('image.image_type = :imageType', { imageType: ImageType.SLIDERTYPE })
            .limit(6)
            .getMany();
    ...
    public async GetImageLogo(): Promise<ImageModel | null> {
        try {
            await super.connectDatabase();
            const repositoryImage: Repository<Image> = super.createRepository(Image) as Repository<Image>;
            const image: Image = await repositoryImage.createQueryBuilder('image')
            .where('image.image_type = :imageType', { imageType: ImageType.LOGOTYPE })
            .getOneOrFail();
            await super.disconnectDatabase();
    ...
    public async InsertImageLogo(imageNew: ImageNew): Promise<Image | boolean> {
        try {
            ...
            await imageRepository.createQueryBuilder('image').delete().where('image.image_type = :imageType', { imageType: ImageType.LOGOTYPE }).execute();
